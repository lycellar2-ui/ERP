    ...
    try {
        ...
        const enrichedProducts = products.map(p => {
            ...
            return {
                ...
                tastingNotes: p.profile ? [
                    p.profile.grapes ? `Giống nho: ${p.profile.grapes}` : null,
                    p.profile.color ? `Màu sắc: ${p.profile.color}` : null,
                    p.profile.aromas ? `Hương thơm: ${p.profile.aromas}` : null,
                    p.profile.palate ? `Vị giác: ${p.profile.palate}` : null,
                    p.profile.servingTemp ? `Phục vụ: ${p.profile.servingTemp}` : null,
                    p.profile.foodPairings ? `Ẩm thực: ${p.profile.foodPairings}` : null,
                ].filter(Boolean).join('. ') : '',
