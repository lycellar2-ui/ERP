...
import { Sparkles, Loader2, RefreshCw, TrendingUp, Target, Users, AlertTriangle, Trophy, XCircle, ChevronDown, ChevronUp, Save, CheckCircle } from 'lucide-react'
...
export function AIPipelineAnalysis() {
    const aiStatus = useAiStatus('pipeline')
    ...
    async function handleSaveReport() {
        ...
        const res = await fetch('/api/ai/reports', {
            ...
            body: JSON.stringify({ module: 'pipeline', title: `Pipeline Analysis — ${new Date().toLocaleDateString('vi-VN')}`, analysis, stats }),
        })
            ...
            <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid #E2E8F0' }}>
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-[10px]" style={{ color: '#64748B' }}>Phân tích như Sales Director — coaching, risk, forecast</p>
                    </div>
                </div>
                <button
            ...
            {!collapsed && <>
                ...
                {stats && !loading && (
                    <div className="grid grid-cols-3 lg:grid-cols-6 gap-2 px-5 pt-4">
                        <div className="text-center p-2 rounded" style={{ background: '#FFFFFF' }}>
                            <div className="flex items-center justify-center gap-1 mb-0.5"><Target size={11} style={{ color: '#0891B2' }} /></div>
                            <p className="text-lg font-bold" style={{ color: '#0891B2' }}>{stats.activeDeals}</p>
                ...
                {!analysis && !loading && !error && (
                    <div className="px-5 py-4 flex flex-col items-center gap-2">
                        <TrendingUp size={20} style={{ color: '#E2E8F0' }} />
