        ...
        >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                    <TrendingUp size={16} className="text-[#87CBB9]" />
                    <h3 className="font-semibold text-sm text-[#E8F1F2]">Biến Động Doanh Số Theo Ngày</h3>
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-[#87CBB9]/10 text-[#87CBB9] border border-[#87CBB9]/20">
                        {items.length} ngày
                    </span>
                </div>
                {peakDay && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                <div className="bg-[#142433] p-3 rounded-md border border-[#2A4355]">
                    <span className="text-[10px] text-[#4A6A7A] uppercase font-semibold block mb-0.5">
                        Tổng Doanh Số Kỳ
                    </span>
                    <span className="text-base font-bold text-[#87CBB9] font-mono">
                ...
                <div className="bg-[#142433] p-3 rounded-md border border-[#2A4355]">
                    ...
                    <span className="text-base font-bold text-[#E8F1F2] font-mono">
                        {totalOrders} đơn
                    </span>
                </div>
                <div className="bg-[#142433] p-3 rounded-md border border-[#2A4355]">
            ...
            {/* ─── Main Chart Visual ─── */}
            {items.length === 0 ? (
                <div className="py-12 text-center text-xs text-[#4A6A7A]">
                    Không có dữ liệu trong khoảng thời gian đã chọn
                </div>
            ) : isSingleDay ? (
                /* Single Day View (e.g. Today / Yesterday) */
                <div className="bg-[#142433] p-5 rounded-lg border border-[#2A4355] text-center space-y-3">
                    <p className="text-xs text-[#8AAEBB] font-medium">
                        Chi tiết ngày <strong className="text-[#E8F1F2]">{items[0].label}</strong> ({items[0].dayOfWeek})
                    </p>
                    <div className="flex items-center justify-center gap-6">
                        <div>
                            <span className="text-[10px] uppercase text-[#4A6A7A] block">Doanh thu ngày</span>
                            <span className="text-2xl font-bold text-[#87CBB9] font-mono">
                                {formatVND(items[0].revenue)}
                            </span>
                        </div>
                        <div className="h-8 w-px bg-[#2A4355]" />
                        <div>
                            <span className="text-[10px] uppercase text-[#4A6A7A] block">Số đơn phát sinh</span>
                            <span className="text-2xl font-bold text-[#E8F1F2] font-mono">
                ...
                <div className="space-y-2 pt-2">
                                ...
                                >
                                    ...
                                    <span
                                        className={`text-[9px] sm:text-[10px] mt-1.5 transition-colors text-center ${
                                            it.isWeekend ? 'text-[#D4A853]/70 font-medium' : 'text-[#4A6A7A]'
                                        } group-hover:text-[#E8F1F2]`}
                                    >
                                        {items.length > 20
                    ...
                    <div
                        ...
                        style={{
                            background: '#142433',
                            borderColor: hoveredItem ? '#87CBB9' : '#2A4355',
                        }}
                    >
                        {hoveredItem ? (
                            <div className="flex items-center justify-between w-full">
                                <div className="flex items-center gap-2">
                                    <span className="font-semibold text-[#E8F1F2]">
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="text-[#8AAEBB]">
                                        Số đơn:{' '}
                                        <strong className="text-[#E8F1F2]">{hoveredItem.orderCount}</strong>
                                    </span>
                                    <span className="text-[#87CBB9] font-bold text-sm font-mono">
                                        {formatVND(hoveredItem.revenue)}
                                    </span>
                                </div>
                            </div>
                        ) : (
                            <div className="text-[11px] text-[#4A6A7A] italic flex items-center gap-1.5">
                                <Clock size={12} />
