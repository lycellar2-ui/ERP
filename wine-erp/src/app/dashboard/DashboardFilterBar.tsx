        ...
        >
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[11px] font-semibold uppercase tracking-wider mr-1 text-[#4A6A7A] flex items-center gap-1">
                        ...
                        return (
                            <button
                                ...
                                className={`px-2.5 py-1 text-xs font-medium rounded-md transition-all flex items-center gap-1.5 ${
                                    active
                                        ? 'bg-[#87CBB9]/20 text-[#87CBB9] border border-[#87CBB9]/40 shadow-sm'
                                        : 'bg-[#142433] text-[#8AAEBB] border border-[#2A4355] hover:text-[#E8F1F2] hover:border-[#4A6A7A]'
                                }`}
                            >
                                {active && <Check size={11} className="text-[#87CBB9]" />}
                                {item.label}
                            </button>
                        )
                    })}
                ...
                <div className="flex items-center gap-2.5 ml-auto flex-wrap">
                    <div className="flex items-center gap-1.5 bg-[#142433] px-2.5 py-1 rounded-md border border-[#2A4355]">
                        <Building2 size={13} className="text-[#87CBB9]" />
                        <span className="text-xs text-[#8AAEBB] font-medium hidden sm:inline">Pháp nhân:</span>
                        <select
                            value={entity}
                            onChange={handleEntityChange}
                            className="bg-transparent text-xs font-semibold text-[#E8F1F2] focus:outline-none cursor-pointer"
                        >
                            <option value="ALL" className="bg-[#142433] text-[#E8F1F2]">
                                Tất cả pháp nhân (Toàn công ty)
                            </option>
                            {legalEntities.map((le) => (
                                <option key={le.id} value={le.id} className="bg-[#142433] text-[#E8F1F2]">
                                    [{le.code}] {le.name}
                                </option>
                            ))}
                        </select>
                    ...
                    {isPending && (
                        <div className="flex items-center gap-1 text-[11px] text-[#87CBB9] font-medium animate-pulse px-2 py-0.5 rounded bg-[#87CBB9]/10">
            ...
            {showCustomModal && (
                <form
                    ...
                    className="flex flex-wrap items-center gap-2 pt-2 border-t border-[#2A4355] text-xs"
                >
                    <span className="text-[#8AAEBB] font-medium flex items-center gap-1">
                    ...
                    <input
                        ...
                        required
                        className="bg-[#142433] border border-[#2A4355] text-[#E8F1F2] px-2 py-1 rounded text-xs focus:border-[#87CBB9] focus:outline-none"
                    />
                    <ArrowRight size={12} className="text-[#4A6A7A]" />
                    <input
                        type="date"
                        value={customTo}
                    ...
                    <button
                        type="submit"
                        className="px-3 py-1 rounded bg-[#5BA88A] hover:bg-[#5BA88A]/80 text-[#0E1A24] font-semibold text-xs transition-all cursor-pointer"
                    >
                        Áp dụng ngày
                    </button>
                    <button
                        ...
                        className="px-2 py-1 text-xs text-[#8AAEBB] hover:text-[#E8F1F2]"
                    >
                        Đóng
                    </button>
                </form>
            ...
            <div className="flex items-center justify-between text-[11px] text-[#4A6A7A] pt-1">
                <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#87CBB9]" />
                    <span>Dữ liệu đang lọc theo:</span>
                    <strong className="text-[#87CBB9]">{displayRangeText}</strong>
                    {entity !== 'ALL' && (
                        <span>
                            · Pháp nhân:{' '}
