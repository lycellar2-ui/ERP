...
export async function confirmSalesOrder(id: string): Promise<{ success: boolean; error?: string; needsApproval?: boolean }> {
    try {
        ...
        const approvalRes = await submitForApproval({
            docType: 'SALES_ORDER',
            docId: id,
            docValue: amount,
        ...
        const targetStatus = (approvalRes.success && !approvalRes.requestId) 
            ? 'PENDING_ACCOUNTING' 
            : 'PENDING_APPROVAL'

        await prisma.salesOrder.update({ where: { id }, data: { status: targetStatus } })
    ...
    try {
        ...
        await prisma.approvalLog.create({
            data: {
                ...
                action: 'APPROVE',
                approvedBy: user.id,
                comment: isCEO ? 'CEO phê duyệt' : 'Sale Admin phê duyệt'
            }
        })
