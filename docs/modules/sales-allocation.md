...
| Tính năng | File code | Ghi chú |
|---|---|---|
| **VNPT Types & Models** | `src/lib/vnpt/types.ts` | Khai báo các interface `VnptConfig`, `InvoicePayload`, `InvoiceItem`, `VnptCallResult`, `VnptDraftMetadata`, `VnptSyncResult`, `InvoiceDateWarning`. |
| **Đa Pháp Nhân (Thắng Ân vs Ly's Cellar)** | `src/lib/vnpt/vnpt-client.ts:getVnptConfigForEntity` | Tách biệt cấu hình tài khoản, mẫu số và ký hiệu theo pháp nhân đơn hàng: Thắng Ân (`TA`: C26TTA / 1/001) và Ly's Cellar (`LC`: C26TLY / 1/001). Đã kết nối và xác thực thành công cả 2 pháp nhân trên Web Service VNPT production. |
