...
## 6. Giao Dịch Nội Bộ & Đa Pháp Nhân (Inter-company & Multi-Entity Constraints)
Dữ liệu của hệ thống dùng chung (Single System) nhưng phân tách Pháp nhân bằng Tag `legal_entity_id`. Hai pháp nhân chính là Thắng Ân (Nhập khẩu) và Lys (Phân phối), dùng chung kho (WMS).
**Luồng Logic (Bán chéo nội bộ để lách thuế SCT):**
1. Lys cần hàng từ Thắng Ân để xuất bán.
2. Thắng Ân tạo SO (Bán cho Lys) -> Hệ thống tự động (Auto-pairing) sinh PO tương ứng bên Lys.
3. Kế toán duyệt giao dịch -> WMS chuyển đổi `owner_entity_id` của các lô `StockLot` từ Thắng Ân sang Lys tại chỗ mà không cần phải luân chuyển vật lý.
4. Phát sinh `ARInvoice` bên Thắng Ân và `APInvoice` bên Lys để đối trừ hoặc thanh toán theo định kỳ.

**Quy Tắc Ràng Buộc Cứng (Cấm Vi Phạm):**
- ❌ **TỪ CHỐI TẠO SO BÁN RA NGOÀI (B2B/B2C)** nếu người dùng chưa chọn Pháp nhân xuất tuyến (`legal_entity_id`).
- ❌ **TỪ CHỐI PICKING (XUẤT KHO)** nếu lô hàng (`StockLot`) không khớp với Pháp nhân (`owner_entity_id` != `SO.legal_entity_id`). Nghĩa là Lys KHÔNG ĐƯỢC phép bán trực tiếp hàng của Thắng Ân nếu chưa mua lại.
